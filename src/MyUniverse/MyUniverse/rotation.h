...
void CreateGlbRotmat(GlbRotmat &r);//生成初始旋转矩阵
void CloneGlbRotmat(GlbRotmat r, GlbRotmat &r_dst);

float Point2Angle(GlbPoint3d p1, GlbPoint3d p2);//计算p1到p2的旋转角度
void Point2Pivot(GlbPoint3d p1, GlbPoint3d p2, GlbPoint3d &pivot, bool bNormalize);//计算p1到p2的旋转轴

void AngleGlbRotmat(GlbPoint3d angle, GlbRotmat &r);//angle的三个分量为绕x，绕y，绕z旋转的角度，依次进行旋转的矩阵
void PointGlbRotmat(GlbPoint3d p1, GlbPoint3d p2, GlbRotmat &r);//计算从p1到p2的旋转矩阵
void PivotRotmat(GlbPoint3d pivot, float angle, GlbRotmat &rot);
void PivotRotPoint(GlbPoint3d p, GlbPoint3d pivot, float angle, GlbPoint3d &p_out);//将p点以pivot轴旋转angle角度
void CreateNormPivot(GlbPoint3d p, GlbPoint3d directPoint, bool bHeadDirect, GlbPoint3d &pivot_h, GlbPoint3d &pivot_v);//生成p点到headPoint的旋转轴，以及其正交方向的旋转轴
